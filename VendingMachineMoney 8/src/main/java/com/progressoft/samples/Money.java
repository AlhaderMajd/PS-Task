...
public class Money {
    ...
    public Money minus(Money other) {
        Money result = this.clone();//1*10
        double remainingAmount = other.amount();//5

        //اذا بدي اطرح ٥ من ٤
        if (this.amount() < remainingAmount) throw new IllegalArgumentException("Result cannot be negative");

        for (Money resultDenom : result.denominationsList) {
            //بقدرش اطرح من 10 بس بقدر اطرح من 1*10 اذا كنت خمسة
            while (remainingAmount >= resultDenom.getDenominationValue() && resultDenom.getDenominationCount() > 0) {
                //نطرح واحد من ترارات العدد الذي نريد ان نطرح منه ففي مثالنا نرح ١ من تكرارات ال ١٠ فتصبح ٩
                resultDenom.setDenominationCount(resultDenom.getDenominationCount() - 1);
                // نخصم قيمة المطروح من المبلغ الاجمالي فيتبقى المبلغ الذي لم يكتمل خصمه
